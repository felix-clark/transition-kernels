...
exactly to 1st order in $\epsilon$, and in fact this holds true for all $0 \leq
	\epsilon \leq 1$.\footnote{We can write $S = 1 + \epsilon T$ and note that it
	appears that $T^2 = -T$, up to maybe some inactive terms. Then with $N$
	applications such that $\lambda = \epsilon N$, $S \sim \left(1 + \lambda
		T/N\right)^{N} \rightarrow e^{\lambda T} = 1 - T(e^{-\lambda}-1)$ and we
	see how composing multiple small-$\epsilon$ kernels simply results in an
	increase of $\epsilon$ beyond the small regime.}
