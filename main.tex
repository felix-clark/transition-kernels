...
function. We'll try to back into it using the formula\footnote{That is,
	\begin{equation} L_n^{(\alpha)}(x) = \frac{e^x x^{-\alpha}}{n!} \frac{d^n}{dx^n}e^{-x}x^{n+\alpha}
	\end{equation}} instead of using the power series.

For $y = x - \gamma x' > 0$,
\begin{align}
	S(w) & \sim \frac{-2\pi i}{2\pi(k-1)!} \lim_{t \rightarrow -i\beta} \frac{\partial^{k-1}}{\partial t ^{k-1}} e^{-ity} (\gamma t + i \beta)^k                                      \\
